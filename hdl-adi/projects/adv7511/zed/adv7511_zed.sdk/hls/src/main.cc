...
//void *mapped_base_reserved_mem[2];
...
while(1) {
	*c >> inFrame;
	...
	//munmap(mapped_base_reserved_mem[0],MAP_SIZE);
	//munmap(mapped_base_reserved_mem[1],MAP_SIZE);
}
    return 0;
}
...
void * setup_reserved_mem(uint input_address) //returns a pointer in userspace to the device
{
    ...
    // be at the start of the page.
        ...
        //mapped_base_reserved_mem[i] = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd_reserved_mem, dev_base & ~MAP_MASK);
    ...
    // that was mapped as memory is mapped at the start of a page
        ...
        //mapped_dev_base = mapped_base_reserved_mem[i] + (dev_base & MAP_MASK);
